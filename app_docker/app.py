...
def get_schedule(conn, selected_league_value, date_range, featured_matches):
    if not featured_matches:
        return pd.DataFrame()  # pusta tabela jeśli brak meczów

    game_ids_sql = ",".join(f"'{gid}'" for gid in featured_matches)
    ...
    schedule = pd.read_sql(query, conn, params=(selected_league_value, date_range[0], date_range[1]))
    return schedule

def get_events(conn, featured_matches):
    if not featured_matches:
        return pd.DataFrame()
...
with st.sidebar:
    with st.sidebar.expander("Filters", expanded=False):
        ...
        two_weeks_ago = today - pd.Timedelta(weeks=2)
    ...
    query = """
        SELECT game_id, date
        FROM schedule
        WHERE league = %s AND date BETWEEN %s AND %s
    """
    filtered_schedule = pd.read_sql(query, st.session_state.conn, params=(selected_league_value, date_range[0], date_range[1]))

    featured_matches = set(filtered_schedule["game_id"])

    schedule = get_schedule(st.session_state.conn, selected_league_value, date_range, featured_matches)
    events = get_events(st.session_state.conn, featured_matches)

    if schedule.empty:
        st.warning("No matches with available statistics were found for the selected date range.")
        st.stop()
